...
<p align="center">
  ...
  <a href="https://github.com/bjxg/aigw/issues">🐛 报告问题</a> ·
  <a href="https://github.com/bjxg/aigw/pulls">✨ 功能请求</a>
</p>
...
aigw 会把 AI CLI 订阅、OAuth 凭据、API Key 以及兼容上游服务整合成一个可管理的 API 层。它可以让 Claude Code、Gemini CLI、OpenAI Codex、Amp CLI、OpenAI 兼容客户端等工具通过统一端点访问多类上游，同时围绕流量提供分组路由、故障转移、请求日志、配额管控、模型价格、生图配置、API Key 自助查询、`/manage` Web 面板托管和终端管理流程。
...
aigw 可以在 `/manage` 暴露内置 Web 控制面板。服务端既可以托管打包后的 SPA 资源，也可以回退到同步的管理面板资源。
...
| 首页概览 | 运维概览 |
| :------- | :------- |
| <img src="docs/images/readme-showcase/home-overview-1.png" width="100%" alt="aigw 首页概览" /> | <img src="docs/images/readme-showcase/home-overview-2.png" width="100%" alt="aigw 运维概览" /> |
...
Docker Compose 是 aigw 推荐的安装方式。仓库内的 `docker-compose.yml` 默认使用已发布的 `ghcr.io/bjxg/aigw:latest` 镜像，并启动 API 服务。
...
如果你使用 Docker Compose 部署，也可以在环境变量中设置 `aigw_LOCALE=en` 或 `aigw_LOCALE=zh`，控制 TUI 的默认语言。
