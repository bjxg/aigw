...
func (w *Watcher) reloadClients(rescanAuth bool, affectedOAuthProviders []string, forceAuthRefresh bool) {
	...
	log.Infof("full client load complete - %d clients (%d Gemini API keys + %d Claude API keys + %d Codex keys + %d OpenAI-compat)",
		geminiAPIKeyCount+claudeAPIKeyCount+codexAPIKeyCount+openAICompatCount,
		geminiAPIKeyCount,
		claudeAPIKeyCount,
		codexAPIKeyCount,
		openAICompatCount,
