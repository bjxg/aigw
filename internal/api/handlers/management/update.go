...
const (
	...
	updaterTokenEnv       = "aigw-server_UPDATER_TOKEN"
	githubTokenEnv        = "aigw-server_GITHUB_TOKEN"
	autoUpdateChannelEnv  = "aigw-server_UPDATE_CHANNEL"
	defaultUpdaterService = "aigw-server"
)
...
func resolveUpdaterURL(cfg *config.Config) string {
	if fromEnv := strings.TrimSpace(os.Getenv("aigw-server_UPDATER_URL")); fromEnv != "" {
		return fromEnv
	}
...
func updaterTargetService() string {
	if service := strings.TrimSpace(os.Getenv("aigw-server_TARGET_SERVICE")); service != "" {
		return service
	}
