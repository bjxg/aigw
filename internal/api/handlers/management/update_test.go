...
func TestBuildUpdateCheckGracefullyHandlesGitHubFailures(t *testing.T) {
	...
	cfg.AutoUpdate.Repository = "https://github.com/bjxg/aigw-server"
	cfg.RemoteManagement.PanelGitHubRepository = "https://github.com/bjxg/aigw-panel"
...
func TestBuildUpdateCheckUsesConfiguredPanelRepository(t *testing.T) {
	...
	if !slices.Contains(repos, "bjxg/aigw-server") {
		t.Fatalf("repos = %v, want backend repo bjxg/aigw-server", repos)
	}
	if !slices.Contains(repos, "router-for-me/Cli-Proxy-API-Management-Center") {
		t.Fatalf("repos = %v, want configured panel repo", repos)
	}
	if slices.Contains(repos, "bjxg/aigw-panel") {
...
func TestBuildCurrentUpdateStateDoesNotQueryGitHub(t *testing.T) {
	...
	cfg.AutoUpdate.Enabled = true
	cfg.AutoUpdate.Channel = "dev"
	cfg.AutoUpdate.DockerImage = "ghcr.io/bjxg/aigw-server"
	handler := &Handler{cfg: cfg}
...
func TestFetchUpdateProgressProxiesUpdaterStatus(t *testing.T) {
	updater := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"status":"running","stage":"pulling","target_version":"dev-abcdef1","logs":[{"timestamp":"2026-04-20T07:30:01Z","stream":"stdout","message":"docker compose pull aigw-server"}]}`))
	}))
	t.Cleanup(updater.Close)
	t.Setenv("aigw-server_UPDATER_URL", updater.URL)
	t.Setenv("aigw-server_UPDATER_TOKEN", "test-token")
	...
	if len(progress.Logs) != 1 || progress.Logs[0].Message != "docker compose pull aigw-server" {
...
func TestFetchBranchCommitUsesGitHubTokenWhenConfigured(t *testing.T) {
	t.Setenv("aigw-server_GITHUB_TOKEN", "test-token")
	...
	info, err := fetchBranchCommit(context.Background(), client, "bjxg/aigw-server", "dev")
	if err != nil {
