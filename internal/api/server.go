...
func NewServer(cfg *config.Config, authManager *auth.Manager, accessManager *sdkaccess.Manager, configFilePath string, opts ...ServerOption) *Server {
	...
	if len(cfg.TrustedProxies) > 0 {
		if err := engine.SetTrustedProxies(cfg.TrustedProxies); err != nil {
			log.Warnf("failed to set trusted proxies %v: %v", cfg.TrustedProxies, err)
		}
	} else {
...
func (s *Server) registerManagementRoutes() {
	...
	mgmt.Use(s.managementAvailabilityMiddleware(), s.mgmt.Middleware(), bodyutil.LimitBodyMiddleware(bodyutil.ManagementBodyLimit))
	{
		...
		mgmt.GET("/users", s.mgmt.GetUsers)
		mgmt.POST("/users", s.mgmt.PostUser)
		mgmt.PUT("/users/:id", s.mgmt.PutUser)
		mgmt.DELETE("/users/:id", s.mgmt.DeleteUser)
	...
	return func(c *gin.Context) {
		...
		if err == nil {
			if result != nil {
				...
				// Resolve API key string to numeric ID for usage DB queries
				if row := usage.GetAPIKey(result.Principal); row != nil {
