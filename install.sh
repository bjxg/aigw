#!/usr/bin/env bash
#
# aigw-server one-click Docker installer
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/bjxg/aigw-server/main/install.sh | bash
#
set -euo pipefail
...
DOCKER_IMAGE_DEFAULT="ghcr.io/bjxg/aigw-server:latest"
CONTAINER_NAME="aigw-server"
DEFAULT_PORT=8317
TOTAL_STEPS=6
OS_NAME="$(uname -s)"
...
case "${OS_NAME}" in
    Darwin)
        DEFAULT_INSTALL_DIR="${HOME}/.aigw-server"
        ;;
    *)
        DEFAULT_INSTALL_DIR="/opt/aigw-server"
        ;;
esac

INSTALL_DIR="${aigw-server_DIR:-$DEFAULT_INSTALL_DIR}"
...
detect_default_locale() {
    local candidate
    candidate="$(normalize_locale "${aigw-server_LOCALE:-}")"
    if [[ -n "$candidate" ]]; then
        echo "$candidate"
        return
    fi
...
load_existing_state() {
    if [[ -f "${INSTALL_DIR}/.env" ]]; then
        ...
        if [[ -n "${aigw-server_LOCALE:-}" ]]; then
            set_locale "${aigw-server_LOCALE}"
        fi
...
# aigw-server configuration file - generated by install.sh
host: ""
port: ${CFG_PORT}
...
aigw-server_CONTAINER_NAME=${CONTAINER_NAME}
aigw-server_INSTALL_DIR=${INSTALL_DIR}
aigw-server_LOCALE=${SCRIPT_LOCALE}
...
services:
  aigw-server:
    ...
    container_name: ${aigw-server_CONTAINER_NAME}
    pull_policy: always
    ports:
      - "${aigw-server_PORT}:${aigw-server_PORT}"
    volumes:
    ...
    environment:
      ...
      LC_ALL: ${aigw-server_LANG}
      PORT: ${aigw-server_PORT}
    healthcheck:
      test: ["CMD-SHELL", "wget -q -O- http://127.0.0.1:${aigw-server_PORT}/ >/dev/null 2>&1 || exit 1"]
      interval: 30s
      timeout: 5s
      retries: 5
      start_period: 20s
...
install_helper_command() {
    ...
    if [[ -d "/usr/local/bin" && -w "/usr/local/bin" ]]; then
        target="/usr/local/bin/aigw-server"
    elif [[ ! -e "/usr/local/bin" ]] && mkdir -p /usr/local/bin 2>/dev/null; then
        target="/usr/local/bin/aigw-server"
    else
        target="${HOME}/.local/bin/aigw-server"
...
if [[ ! -f "\${COMPOSE_FILE}" ]] || [[ ! -f "\${ENV_FILE}" ]]; then
  echo "aigw-server is not installed at \${INSTALL_DIR}." >&2
  exit 1
fi
...
load_env() {
  ...
  aigw-server_LOCALE="\$(normalize_locale "\${aigw-server_LOCALE:-en}")"
  if [[ -z "\${aigw-server_LOCALE}" ]]; then
    aigw-server_LOCALE="en"
  fi
}
...
status_cmd() {
  ...
  port="\$(extract_port)"
  port="\${port:-\${aigw-server_PORT:-8317}}"
  secret="\$(extract_secret)"
  root_code="\$(http_code "http://127.0.0.1:\${port}/")"
  panel_code="\$(http_code "http://127.0.0.1:\${port}/manage")"

  say "aigw-server 部署状态" "aigw-server deployment status"
  echo "  Install Dir : \${INSTALL_DIR}"
  echo "  Image       : \${CLI_PROXY_IMAGE}"
  echo "  Platform    : \${CLI_PROXY_PLATFORM}"
  echo "  Locale      : \${aigw-server_LOCALE}"
  echo "  Port        : \${port}"
  echo "  Container   : \${aigw-server_CONTAINER_NAME}"
  echo "  API /       : HTTP \${root_code}"
  echo "  Panel /manage: HTTP \${panel_code}"

  if docker inspect "\${aigw-server_CONTAINER_NAME}" >/dev/null 2>&1; then
    echo "  Runtime     : \$(docker inspect -f '{{if .State.Health}}{{.State.Health.Status}}{{else}}{{.State.Status}}{{end}}' "\${aigw-server_CONTAINER_NAME}")"
    echo "  Started At  : \$(docker inspect -f '{{.State.StartedAt}}' "\${aigw-server_CONTAINER_NAME}")"
  else
...
tui_cmd() {
  ...
  exec docker exec -e aigw-server_LOCALE="\${aigw-server_LOCALE}" -it "\${aigw-server_CONTAINER_NAME}" /CLIProxyAPI/CLIProxyAPI -config /CLIProxyAPI/config.yaml -password "\${secret}" -tui
}

help_cmd() {
  load_env
  say "用法: aigw-server <status|update|restart|logs|tui>" "Usage: aigw-server <status|update|restart|logs|tui>"
}
...
show_result() {
    ...
    if is_zh; then
        ...
        echo ""
        echo -e "  ${C_GREEN}${SYM_STAR}${C_RESET} ${C_BOLD}aigw-server 已成功部署${C_RESET}"
    else
        echo -e "  ${C_BG_GREEN}${C_WHITE}${C_BOLD} Deployment Complete ${C_RESET}"
        echo ""
        echo -e "  ${C_GREEN}${SYM_STAR}${C_RESET} ${C_BOLD}aigw-server has been deployed successfully${C_RESET}"
    fi
    echo ""
    echo "  Install Dir : ${INSTALL_DIR}"
    ...
    if is_zh; then
        ...
        echo -e "    ${C_YELLOW}aigw-server status${C_RESET}   查看部署状态"
        echo -e "    ${C_YELLOW}aigw-server update${C_RESET}   拉取最新镜像并更新"
        echo -e "    ${C_YELLOW}aigw-server restart${C_RESET}  重启服务"
        echo -e "    ${C_YELLOW}aigw-server logs${C_RESET}     查看实时日志"
        echo -e "    ${C_YELLOW}aigw-server tui${C_RESET}      进入容器内 TUI（语言随部署配置）"
    else
        echo -e "  ${C_DIM}Common commands:${C_RESET}"
        echo -e "    ${C_YELLOW}aigw-server status${C_RESET}   Show deployment status"
        echo -e "    ${C_YELLOW}aigw-server update${C_RESET}   Pull the latest image and update"
        echo -e "    ${C_YELLOW}aigw-server restart${C_RESET}  Restart the service"
        echo -e "    ${C_YELLOW}aigw-server logs${C_RESET}     Tail service logs"
        echo -e "    ${C_YELLOW}aigw-server tui${C_RESET}      Open the container TUI in the selected language"
    fi
    echo ""
}
...
uninstall() {
    ...
    banner
    if is_zh; then
        echo -e "  ${C_BG_RED}${C_WHITE}${C_BOLD} 卸载 aigw-server ${C_RESET}"
        yn="$(prompt_input "  $(echo -e "${C_RED}?${C_RESET}") 确认卸载? 配置和数据将被删除 [y/N]: " "N")" || yn="N"
    else
        echo -e "  ${C_BG_RED}${C_WHITE}${C_BOLD} Uninstall aigw-server ${C_RESET}"
    ...
    case "$yn" in
        [Yy]*)
            ...
            rm -rf "${INSTALL_DIR}"
            rm -f "/usr/local/bin/aigw-server" "${HOME}/.local/bin/aigw-server" 2>/dev/null || true
            if is_zh; then
                success "aigw-server 已完全卸载"
            else
                success "aigw-server has been removed completely"
            fi
            ;;
        *)
            if is_zh; then
...
main() {
    ...
    if wait_for_ready; then
        ...
        if is_zh; then
            warn "服务未在预期时间内完成启动，可稍后执行 aigw-server status / aigw-server logs 检查。"
        else
            warn "Service did not become ready in time. Run aigw-server status or aigw-server logs to inspect it."
        fi
    fi
...
case "${1:-}" in
    ...
    --help|-h|help)
        ...
        if is_zh; then
            ...
            echo ""
            echo "  一键安装:"
            echo "    curl -fsSL https://raw.githubusercontent.com/bjxg/aigw-server/main/install.sh | bash"
        else
            echo "  Usage:"
