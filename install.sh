#!/usr/bin/env bash
#
# aigw one-click Docker installer
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/bjxg/aigw/main/install.sh | bash
#
set -euo pipefail
...
case "${OS_NAME}" in
    Darwin)
        DEFAULT_INSTALL_DIR="${HOME}/.aigw"
        ;;
    *)
        DEFAULT_INSTALL_DIR="/opt/aigw"
        ;;
esac
...
# aigw configuration file - generated by install.sh
host: ""
port: ${CFG_PORT}
...
if [[ ! -f "\${COMPOSE_FILE}" ]] || [[ ! -f "\${ENV_FILE}" ]]; then
  echo "aigw is not installed at \${INSTALL_DIR}." >&2
  exit 1
fi
...
help_cmd() {
  load_env
  say "用法: aigw <status|update|restart|logs|tui>" "Usage: aigw <status|update|restart|logs|tui>"
}
...
show_result() {
    ...
    if is_zh; then
        ...
        echo ""
        echo -e "  ${C_GREEN}${SYM_STAR}${C_RESET} ${C_BOLD}aigw 已成功部署${C_RESET}"
    else
        echo -e "  ${C_BG_GREEN}${C_WHITE}${C_BOLD} Deployment Complete ${C_RESET}"
        echo ""
        echo -e "  ${C_GREEN}${SYM_STAR}${C_RESET} ${C_BOLD}aigw has been deployed successfully${C_RESET}"
    fi
    echo ""
    echo "  Install Dir : ${INSTALL_DIR}"
    ...
    if is_zh; then
        ...
        echo -e "    ${C_YELLOW}aigw status${C_RESET}   查看部署状态"
        echo -e "    ${C_YELLOW}aigw update${C_RESET}   拉取最新镜像并更新"
        echo -e "    ${C_YELLOW}aigw restart${C_RESET}  重启服务"
        echo -e "    ${C_YELLOW}aigw logs${C_RESET}     查看实时日志"
        echo -e "    ${C_YELLOW}aigw tui${C_RESET}      进入容器内 TUI（语言随部署配置）"
    else
        echo -e "  ${C_DIM}Common commands:${C_RESET}"
        echo -e "    ${C_YELLOW}aigw status${C_RESET}   Show deployment status"
        echo -e "    ${C_YELLOW}aigw update${C_RESET}   Pull the latest image and update"
        echo -e "    ${C_YELLOW}aigw restart${C_RESET}  Restart the service"
        echo -e "    ${C_YELLOW}aigw logs${C_RESET}     Tail service logs"
        echo -e "    ${C_YELLOW}aigw tui${C_RESET}      Open the container TUI in the selected language"
    fi
    echo ""
}
...
uninstall() {
    ...
    banner
    if is_zh; then
        echo -e "  ${C_BG_RED}${C_WHITE}${C_BOLD} 卸载 aigw ${C_RESET}"
        yn="$(prompt_input "  $(echo -e "${C_RED}?${C_RESET}") 确认卸载? 配置和数据将被删除 [y/N]: " "N")" || yn="N"
    else
        echo -e "  ${C_BG_RED}${C_WHITE}${C_BOLD} Uninstall aigw ${C_RESET}"
    ...
    case "$yn" in
        [Yy]*)
            ...
            rm -rf "${INSTALL_DIR}"
            rm -f "/usr/local/bin/aigw" "${HOME}/.local/bin/aigw" 2>/dev/null || true
            if is_zh; then
                success "aigw 已完全卸载"
            else
                success "aigw has been removed completely"
            fi
            ;;
        *)
            if is_zh; then
...
main() {
    ...
    if wait_for_ready; then
        ...
        if is_zh; then
            warn "服务未在预期时间内完成启动，可稍后执行 aigw status / aigw logs 检查。"
        else
            warn "Service did not become ready in time. Run aigw status or aigw logs to inspect it."
        fi
    fi
...
case "${1:-}" in
    ...
    --help|-h|help)
        ...
        if is_zh; then
            ...
            echo ""
            echo "  一键安装:"
            echo "    curl -fsSL https://raw.githubusercontent.com/bjxg/aigw/main/install.sh | bash"
        else
            echo "  Usage:"
