...
const (
	...
	defaultComposeFile   = ""
	defaultEnvFile       = ""
	defaultTargetService = "aigw-server"
...
func main() {
	cfg := updaterConfig{
		Addr:           envOrDefault("aigw-server_UPDATER_ADDR", defaultListenAddr),
		Token:          strings.TrimSpace(os.Getenv("aigw-server_UPDATER_TOKEN")),
		ComposeFile:    envOrDefault("aigw-server_COMPOSE_FILE", defaultComposeFile),
		EnvFile:        envOrDefault("aigw-server_ENV_FILE", defaultEnvFile),
		ProjectName:    strings.TrimSpace(os.Getenv("aigw-server_COMPOSE_PROJECT_NAME")),
		DefaultService: envOrDefault("aigw-server_TARGET_SERVICE", defaultTargetService),
	...
	log.Printf("aigw-server updater listening on %s", cfg.Addr)
