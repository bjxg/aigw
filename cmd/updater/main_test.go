...
func TestUpdaterRejectsInvalidBearerToken(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/v1/update", strings.NewReader(`{"service":"aigw-server"}`))
...
func TestUpdaterPersistsRequestedImageBeforeComposeUpdate(t *testing.T) {
	...
	if err := os.WriteFile(envFile, []byte("CLI_PROXY_IMAGE=ghcr.io/bjxg/aigw-server:dev\nOTHER=value\n"), 0o600); err != nil {
	...
	server := newUpdaterServer(updaterConfig{
		...
		Runner: func(_ context.Context, _ string, _ string, _ string, _ string, reporter updateReporter) error {
			...
			content := string(data)
			if !strings.Contains(content, "CLI_PROXY_IMAGE=ghcr.io/bjxg/aigw-server:latest\n") {
	...
	req := httptest.NewRequest(
		http.MethodPost,
		"/v1/update",
		strings.NewReader(`{"service":"cli-proxy-api","image":"ghcr.io/bjxg/aigw-server","tag":"latest"}`),
	)
	rec := httptest.NewRecorder()
...
func TestUpdaterRejectsRequestWhenEnvFileCannotBeUpdated(t *testing.T) {
	...
	req := httptest.NewRequest(
		http.MethodPost,
		"/v1/update",
		strings.NewReader(`{"service":"cli-proxy-api","image":"ghcr.io/bjxg/aigw-server","tag":"dev"}`),
	)
	rec := httptest.NewRecorder()
...
func TestUpdaterStatusExposesTargetStageAndLogs(t *testing.T) {
	...
	server := newUpdaterServer(updaterConfig{
		...
		Runner: func(_ context.Context, _ string, _ string, _ string, service string, reporter updateReporter) error {
			...
			reporter.Log("stderr", "Container aigw-server Recreated")
			return nil
		},
	})
	...
	req := httptest.NewRequest(
		http.MethodPost,
		"/v1/update",
		strings.NewReader(`{"service":"aigw-server","image":"ghcr.io/bjxg/aigw-server","tag":"dev","version":"dev-abcdef1","commit":"abcdef123456","channel":"dev"}`),
	)
	rec := httptest.NewRecorder()
	server.handleUpdate(rec, req)
	if rec.Code != http.StatusAccepted {
	...
	if len(payload.Logs) != 1 || payload.Logs[0].Message != "docker compose pull aigw-server" {
...
func TestUpdaterFailsWhenComposePullSkipsTargetService(t *testing.T) {
	...
	req := httptest.NewRequest(
		http.MethodPost,
		"/v1/update",
		strings.NewReader(`{"service":"cli-proxy-api","image":"ghcr.io/bjxg/aigw-server","tag":"dev","version":"dev-6704f60","commit":"6704f60ee834bce20e22fc65e67868801f483e32","channel":"dev"}`),
	)
	rec := httptest.NewRecorder()
	server.handleUpdate(rec, req)
	if rec.Code != http.StatusAccepted {
